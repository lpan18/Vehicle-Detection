...
def hard_negative_mining(predicted_prob, gt_label, neg_pos_ratio=3.0):
    ...
    :return:
    """
    pos_flag = gt_label > 0                                        # return 0 or 1 for each item, 0 = negative label
...
class MultiboxLoss(nn.Module):
    ...
    def forward(self, confidence, pred_loc, gt_class_labels, gt_bbox_loc):
        ...
        conf_loss = F.cross_entropy(sel_conf.reshape(-1, num_classes), gt_class_labels[sel_flag], reduction='sum') / num_pos.float()

        # Loss for the bounding box prediction
        loc_huber_loss = None
        # TODO: implementation on bounding box regression
        pred_loc = pred_loc[pos_flag].view(-1, 4)
        gt_loc = gt_bbox_loc[pos_flag].view(-1, 4)
        loc_huber_loss = F.smooth_l1_loss(pred_loc, gt_loc, reduction='sum') / num_pos.float()
